...
class SessionMiddleware(Middleware):
    ...
    def _exec(self, args):
        ...
        timestamp = self._ctx.cache.get(f'session.{user_id}.timestamp')
        if timestamp is not None and datetime.now() - datetime.fromtimestamp(timestamp) < SESSION_LIFETIME:
            self._ctx.session = self._ctx.cache[f'session.{user_id}']
        else:
            ...
            self._ctx.cache[f'session.{user_id}'] = self._ctx.session
            self._ctx.cache[f'session.{user_id}.timestamp'] = time.time()
